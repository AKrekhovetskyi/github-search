...
def main() -> None:
    ...
    github = GitHub()
    html = github.request_page_html(url=f"{github.base_url}/search", proxies=params.proxies, params=params.to_dict())
    urls = github.extract_urls(html)
    for url in urls:
        extra_info = github.extract_extra_info(url["url"], proxies=params.proxies)
        if extra_info:
